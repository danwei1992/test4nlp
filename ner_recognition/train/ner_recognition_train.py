...
import test4nlp.ner_recognition.config.ner_recognition_config as config
...
# 标注数据
train_data = load_data(os.path.join(config.data_path, 'example.train'))
valid_data = load_data(os.path.join(config.data_path, 'example.dev'))
test_data = load_data(os.path.join(config.data_path, 'example.test'))
...
output_layer = 'Transformer-%s-FeedForward-Norm' % (config.bert_layers - 1)
output = model.get_layer(output_layer).output
output = Dense(num_labels)(output)
CRF = ConditionalRandomField(lr_multiplier=config.crf_lr_multiplier)
output = CRF(output)
