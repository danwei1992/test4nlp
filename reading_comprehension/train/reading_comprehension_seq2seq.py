...
import test4nlp.reading_comprehension.config.reading_comprehension_config as config

# 读取数据
def getdata(data_path):
    webqa_data = json.load(open(os.path.join(data_path, 'WebQA.json'), encoding='utf-8'))
    sogou_data = json.load(open(os.path.join(data_path, 'SogouQA.json'), encoding='utf-8'))
    return webqa_data, sogou_data

webqa_data, sogou_data =getdata(config.data_path)

# 保存一个随机序（供划分valid用）
if not os.path.exists(os.path.join(config.data_path, 'random_order.json')):
    random_order = list(range(len(sogou_data)))
    np.random.shuffle(random_order)
    json.dump(random_order, open(os.path.join(config.data_path, 'random_order.json'), 'w'), indent=4)
else:
    random_order = json.load(open(os.path.join(config.data_path, 'random_order.json')))
    ...
    def __iter__(self, random=False):
        ...
        for is_end, D in self.sample(random):
            ...
            for answer in answers:
                if all([
                    a in passage[:config.max_p_len - 2] for a in answer.split(' ')
                ]):
                    final_answer = answer.replace(' ', ',')
                    break
            qa_token_ids, qa_segment_ids = tokenizer.encode(
                question, final_answer, maxlen=config.max_qa_len + 1
            )
            p_token_ids, p_segment_ids = tokenizer.encode(
                passage, maxlen=config.max_p_len
            )
...
class ReadingComprehension(AutoRegressiveDecoder):
    ...
    def answer(self, question, passages, topk=1):
        ...
        for passage in passages:
            passage = re.sub(u' |、|；|，', ',', passage)
            p_token_ids = tokenizer.encode(passage, maxlen=config.max_p_len)[0]
            q_token_ids = tokenizer.encode(question, maxlen=config.max_q_len + 1)[0]
